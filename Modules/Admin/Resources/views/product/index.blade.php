		...
		<div class="col-sm-12">
			<form class="form-inline" action="" style="margin-bottom: 20px;">
				<div class="form-group">
					<input type="text" class="form-control" id="name" placeholder="Tên sản phẩm ..." name="name" value="{{ \Request::get('name') }}">
				</div>
					...
					<select name="cate" id="" class="form-control">
						...
						@if(isset($categories))
							@foreach($categories as $category)
								<option value="{{ $category->id }}" {{ \Request::get('cate') == $category->name ? "selected='selected'" : "" }}>{{ $category->c_name }}</option>
							@endforeach
						@endif
					</select>
				</div>
				...
				@if(isset($products))
					@foreach($products as $product)
						...
						<tr>
							...
							<td>
								<img src="{{ pare_url_file($product->pro_avatar) }}" alt="" class="img img-responsive" width="80px" height="88px">
							</td>
							<td>
							...
							<td>
								<a style="padding: 5px 10px; border: 1px solid #eee; font-size: 12px;" href="{{ route('admin.get.edit.product',$product->id) }}"><i class="fas fa-edit" style="font-size: 11px;"></i>Cập nhật</a>
